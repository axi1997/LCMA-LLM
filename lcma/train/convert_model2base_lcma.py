...
from lcma.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN
from torch.utils.data import Dataset
from lcma.train.lcma_trainer import LCMATrainer

from lcma import conversation as conversation_lib
from lcma.model import *
from lcma.mm_utils import tokenizer_image_token
...
def train():
    ...
    if model_args.vision_tower is not None:
        # lcma config: load llm config from pretrained model
        if "phi3" in model_args.model_name_or_path or "phi-3" in model_args.model_name_or_path:
            config = LCMAPhi3Config.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
        elif "phi2" in model_args.model_name_or_path or "phi-2" in model_args.model_name_or_path:
            config = LCMAPhiConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
        elif "gemma" in model_args.model_name_or_path:
            config = LCMAGemmaConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
        else:
            raise NotImplemented

        # vision config
        if "clip" in model_args.vision_tower:
            vision_config = CLIPVisionConfig.from_pretrained(model_args.vision_tower)
        elif "siglip" in model_args.vision_tower:
            vision_config = SiglipVisionConfig.from_pretrained(model_args.vision_tower)
        elif "dinov2" in model_args.vision_tower:
            vision_config = Dinov2Config.from_pretrained(model_args.vision_tower)
        else:
            raise ValueError(
                f"Unsupported vision tower: {model_args.vision_tower}, please assert clip, siglip or dinov2 in vision_tower name")

        # lcma vision config
        lcma_vis_config = LCMAVisionConfig(**vision_config.to_dict())

        # lcma config: update lcma config using lcma vision config
        config.vision_config["vision_tower"] = lcma_vis_config.to_dict()
        ...
        config.vision_config["vision_tower"]["vision_model_name_or_path"] = model_args.vision_tower

        config.vision_config["mm_projector"]["mm_projector_type"] = projector_args.mm_projector_type
        config.vision_config["mm_projector"]["mm_hidden_size"] = lcma_vis_config.hidden_size
        config.vision_config["mm_projector"]["hidden_size"] = config.hidden_size

        # load lcma, this would only load the slm part of the model, the rest of the model will be initialized later
        if "phi3" in model_args.model_name_or_path or "phi-3" in model_args.model_name_or_path:
            model = LCMAPhi3ForCausalLM.from_pretrained(
            ...
            model = LCMAPhiForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
                trust_remote_code=True,
            ...
            model = LCMAGemmaForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
                trust_remote_code=True,
        ...
        if "clip" in model_args.vision_tower:
            ...
            raise ValueError("vision_tower should be clip or siglip")
        if "dinov2" in model_args.vision_tower:
            model.get_model().vision_tower.dinov2.load_state_dict(clip_or_siglip_model_param, strict=True)
        else:
            model.get_model().vision_tower.load_state_dict(clip_or_siglip_model_param, strict=False)
    else:
        raise ValueError("LCMA only support multimodal models. Please specify vision_tower in model_args.")
    ...
    if 'phi' or "gemma" in model_args.model_name_or_path:
    ...
    if model_args.vision_tower is not None:
        ...
        if "clip" in model_args.vision_tower:
            ...
            data_args.image_processor = BitImageProcessor.from_pretrained(model_args.vision_tower)
    ...
    trainer = LCMATrainer(model=model,
                           tokenizer=tokenizer,
                           args=training_args,
                           **data_module)
    # integrate the MLLM
