...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # 输出当前处理的数据项id
        # print(f"Processing data item with ID: {sources['id']}")
        ...
        if 'image' in sources[0]:
            ...
            try:
                ...
                # 处理加载错误，输出错误图像的路径
                print(f"Error loading image: {os.path.join(image_folder, image_file)}")
                print(f"Error details: {e}")
                # 可以选择返回一个默认的图像或者跳过该图像
                image = None  # 或者返回一个占位图像（如全黑图像）
            ...
            if image:
                ...
                # 如果图像加载失败，您可以在这里做其他处理
                image = torch.zeros(3, self.data_args.image_processor.crop_size['height'],
                                    self.data_args.image_processor.crop_size['width'])
