...
from lcma.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="cuda",
    ...
    if "phi3" in model_name.lower() or "phi-3" in model_name.lower():
        print("load LCMA-Phi3 MSLM!!!")
        config = LCMAPhi3Config.from_pretrained(model_path, trust_remote_code=True)
        ...
        print("load LCMA-Phi2 MSLM!!!")
        config = LCMAPhiConfig.from_pretrained(model_path, trust_remote_code=True)
        ...
        print("load LCMA-Phi1.5 MSLM!!!")
        config = LCMAPhi15Config.from_pretrained(model_path, trust_remote_code=True)
        model = LCMAPhi15ForCausalLM.from_pretrained(
            model_path,
            config=config,
            use_safetensors=True,
            **kwargs).to("cuda")
    elif "gemma" in model_name.lower():
        print("load LCMA-Gemma MSLM!!!")
        config = LCMAGemmaConfig.from_pretrained(model_path, trust_remote_code=True)
